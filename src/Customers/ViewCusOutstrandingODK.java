...
public class ViewCusOutstrandingODK extends javax.swing.JFrame {

    Map<String, Integer> sup = new HashMap();

    public ViewCusOutstrandingODK() {
    ...
    private void initComponents() {
        ...
        setTitle("View Customer Outstandings");

        jLabel3.setFont(new java.awt.Font("Noto Sans", 1, 14)); // NOI18N
        jLabel3.setText("Select Type");

        comboSup.setModel(new javax.swing.DefaultComboBoxModel<>(new String[] { "Select", "Customer Outstadings" }));
...
//            }
            ...
            try {
                ...
                Double ttlInvoiceAmount = 0.0;
                Double ttlinvpaymnts = 0.0;

                Session openSession = PoolManager.getSessionFactory().openSession();

                Criteria cr = openSession.createCriteria(ORM.Customer.class);
                cr.add(Restrictions.eq("isDelete", "Active"));

                //     cr.add(Restrictions.eq("contact1", "0111111111"));
                List<Customer> list = cr.list();

                for (Customer customer : list) {

                    Iterator it = customer.getCustomerHasInvoices().iterator();
                    while (it.hasNext()) {
                        CustomerHasInvoice next = (CustomerHasInvoice) it.next();

                        ttlInvoiceAmount += next.getInvoice().getNetTotal();

                        Iterator it2 = next.getInvoice().getInvoicePayments().iterator();
                        while (it2.hasNext()) {
                            InvoicePayment next1 = (InvoicePayment) it2.next();

                            ttlinvpaymnts += next1.getPaymentAmount();

                        }

                    }

                    System.out.println(ttlInvoiceAmount + customer.getName());
                    System.out.println(ttlinvpaymnts + "ttlinvpaymnts" + customer.getName());

                    if (ttlInvoiceAmount > ttlinvpaymnts) {

                        Vector v = new Vector();
                        v.add(customer.getName());
                        v.add(customer.getContact1());
                        v.add(ttlInvoiceAmount);
                        v.add(ttlinvpaymnts);
                        v.add(ttlInvoiceAmount - ttlinvpaymnts);
...
//        
        if (comboSup.getSelectedIndex() == 2) {

        } else if (comboSup.getSelectedIndex() == 1) {
            ...
            try {
                ...
                cr.add(Restrictions.eq("isDelete", "Active"));
                cr.add(Restrictions.eq("idCustomer", cusid));
...
//
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
        try {
            ...
            Logger.getLogger(ViewCusOutstrandingODK.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    public static void main(String args[]) {
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new ViewCusOutstrandingODK().setVisible(true);
            }
        });
    }
